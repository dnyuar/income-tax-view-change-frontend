...
case class IncomeTaxAgentUser(enrolments: Enrolments, affinityGroup: Option[AffinityGroup], confidenceLevel: ConfidenceLevel) {
  lazy val arn: Option[String] = getEnrolmentId(Constants.agentServiceEnrolmentName)
  ...
  def clientUtr(implicit request: Request[AnyContent]): Option[String] =
	...
	def clientName(implicit request: Request[AnyContent]): Name = {
		Name(request.session.get(ITSASessionKeys.FirstName), request.session.get(ITSASessionKeys.LastName))
	}

  private def getEnrolmentId(key: String) = enrolments.enrolments.collectFirst {
